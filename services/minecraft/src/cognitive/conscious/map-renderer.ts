...
function renderTopDown(bot: Bot, options: Required<MapOptions>): MapResult {
  ...
  const size = r * 2 + 1
  const grid = Array.from({ length: size }, () => Array.from<string>({ length: size }).fill(' '))
  const elevations = Array.from({ length: size }, () => Array.from<number | null>({ length: size }).fill(null))
...
function renderCrossSection(bot: Bot, options: Required<MapOptions>): MapResult {
  ...
  const grid = Array.from({ length: height }, () => Array.from<string>({ length: width }).fill(' '))
  const usedCategories = new Set<BlockCategory>()
